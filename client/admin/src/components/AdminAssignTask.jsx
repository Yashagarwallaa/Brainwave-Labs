...
function AdminAssignTask() {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get(`https://brainwavelabsbackend-jnzx.onrender.com/api/admin/getTasks/${projectCode}`);
  ...
  const checkAndMarkMissedTasks = async () => {
    ...
    for (let task of tasks) {
      ...
      if (task.status === 'Pending' && currentDate > taskDeadline) {
        try {
          await axios.put(`https://brainwavelabsbackend-jnzx.onrender.com/api/admin/updateTaskStatus/${task._id}`, {
            status: 'Missed',
          });
          setTasks((prevTasks) =>
            prevTasks.map((t) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://brainwavelabsbackend-jnzx.onrender.com/api/admin/postTask', newTask);
  ...
  const handleAcceptDeadline = async (taskId, requestedDeadline) => {
    try {
      await axios.put(`https://brainwavelabsbackend-jnzx.onrender.com/api/admin/updateTaskDeadline/${taskId}`, {
  ...
  const handleRejectDeadline = async (taskId) => {
    try {
      await axios.put(`https://brainwavelabsbackend-jnzx.onrender.com/api/admin/rejectTaskDeadline/${taskId}`, {
        requestStatus: 'No',
      });
      setTasks((prevTasks) =>
        prevTasks.map((task) =>
